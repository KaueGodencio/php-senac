...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $foto = null;
    if($erro === '' && isset($_FILES['foto']) && $_FILES['foto']['erro'] !== UPLOAD_ERR_NO_FILE ) {
    // Verifica se houve erro no upload (contantes nativas UPLOAD_ERR_)
        ...
        $erro = 'Erro ao enviar a imagem.';
            ...
            if ($_FILES['foto']['size'] > 2 * 1024 * 1024 ) {
                $foto ='Imagem muito grande (máx. 2MB)';
            }
            if ($erro === '') {
                // finfo = classe nativa do PHP usada para descobrir o tipo real do arquivo
                $finfo = new finfo(FILEINFO_MIME_TYPE);
                // (MINE = tipo do arquivo, ex: imge/jpeg, image/png, application/pdf, etc.)
